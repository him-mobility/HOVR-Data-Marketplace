...
- `import { REGION_PROJECTS } from "@/lib/schema";` (server 전용 recommend.ts를 import하면 better-sqlite3가 클라 번들로 들어가 빌드 실패 → 반드시 schema에서).
...
- 폼 5섹션: 01 직업(칩+직접입력) · 02 활용목적(복수) · **03 관심 지역(단일, REGIONS)** · 04 관심 항목(복수, INTERESTS) · 05 세부 내용(textarea).
- submit:
  ```ts
  ...
  await fetch("/api/recommend", { method:"POST", headers:{"Content-Type":"application/json"},
    body: JSON.stringify({ occupation, purposes, details:fullDetails,
      region: region!=="전체"?region:undefined, projects:selectedProjects }) });
  ```
- 결과: 헤더 `AI 추천` + "비즈니스에 맞는 데이터, AI가 직접 찾아드립니다"(다크 navy 히어로). 카드: 이름 + `약 {totalCount}건`(틸 뱃지) + 설명 + `💡 {reason}`(틸 좌보더) + 태그 + 버튼 [`지도에서 살펴보기 →`(`/demo?${demoQuery}`), `구독 문의`(mailto)]. `aiPowered`면 "AI 맞춤 추천" 아니면 "추천(규칙 기반)".

## D. 검증 포인트

- 지역=부산 선택 + "도로 유지보수" → 추천명에 "부산", `totalCount`=부산만(예: 포트홀 ~700), `demoQuery=events=pothole&projects=busan`. 전국(미지정) 대비 정확히 좁혀짐.
- 키 없는 환경 → 규칙 폴백도 지역 태그·projects 반영.
